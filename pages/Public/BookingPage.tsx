...
import { MOCK_TOURS, MOCK_DEPARTURES, MOCK_ROUTES } from '../../services/mockData';
import { Departure, Passenger } from '../../types';
import { Button } from '../../components/ui/Button';
import { Card } from '../../components/ui/Card';
import { Input, Select } from '../../components/ui/Input';
import { Calendar, User, MapPin, CheckCircle, CreditCard, ChevronRight, ChevronLeft, AlertCircle } from 'lucide-react';
import { Badge } from '../../components/ui/Badge';

// --- Step Components ---

const StepDeparture: React.FC<{ 
  tourId: string, 
  onSelect: (dep: Departure) => void, 
  selectedId?: string 
}> = ({ tourId, onSelect, selectedId }) => {
  const departures = MOCK_DEPARTURES.filter(d => d.tour_id === tourId);
...
const StepPickup: React.FC<{
  onSelect: (id: string) => void,
  selectedId?: string
}> = ({ onSelect, selectedId }) => {
  // Mock fetching routes for the selected departure (assuming associated with tour)
  const route = MOCK_ROUTES[0]; // Simplification for mock
  ...
  const handleNext = () => {
    if (canProceed()) {
      if (step === 4) {
        ...
        navigate(`/p/vertical-tur`); // Redirect to home
      } else {
        setStep(step + 1);
        window.scrollTo(0, 0);
      }
